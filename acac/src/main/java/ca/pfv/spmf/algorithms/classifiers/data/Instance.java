...
public class Instance implements Serializable {
    ...
    public Short getAttributeValue(int k) {
		return items[k];
	}
	...
	public Instance removeLastAttribute() {

		if (items.length <= 1) {
				throw new IllegalArgumentException("Instance phải có ít nhất 2 cột để có thể loại bỏ cột cuối cùng.");
		}

    	Short[] newItems = new Short[items.length - 1];
		System.arraycopy(items, 0, newItems, 0, items.length - 1);

		return new Instance(newItems);
}
